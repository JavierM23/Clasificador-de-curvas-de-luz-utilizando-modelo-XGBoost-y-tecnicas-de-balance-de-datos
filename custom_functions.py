...
def json_save(file_name: str, dict):
...
def knn_imbalance(X, y):
    nbrs = NearestNeighbors(n_neighbors=6, algorithm='ball_tree').fit(X)
    _, indices = nbrs.kneighbors(X)
    tipification = np.zeros(len(X))
    for i, neighbor in enumerate(indices):
        for j in range(1,6):
            if y[neighbor[0]]==y[neighbor[j]]:
                tipification[i]+=1
    instances_type = []
    type_dict = {5:'S', 4:'S', 3:'B', 2:'B', 1:'R', 0:'O'}
    for inst in tipification:
        instances_type.append(type_dict[inst])
    labels = pd.DataFrame(data=y.values, columns=['label'], index=X.index)
    labels['type']=instances_type
    df_return = pd.DataFrame()
    for name in labels['label'].unique():
        df_row = labels[labels['label']==name].groupby('type').count()
        df_row = df_row.T.rename(index={'label': name})
        df_return = df_return.append(df_row)
    df_return.index.name = 'class'
    df_return.fillna(0, inplace=True)
    df_return['total']=df_return.sum(axis=1).astype('int')
    df_return = df_return.astype('int')
    return df_return
